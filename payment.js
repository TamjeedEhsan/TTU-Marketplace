...
document.querySelector("#backLink").href = fromCart ? "cart.html" : `product.html?id=${Number(firstLine.id)}`;
document.querySelector("#editLink").href = fromCart ? "cart.html" : `product.html?id=${Number(firstLine.id)}`;
document.querySelector("#orderItems").innerHTML = lines
  ...
  <div>
    ...
    <small>Seller: ${escapeHTML(line.seller)}</small>
  </div>
