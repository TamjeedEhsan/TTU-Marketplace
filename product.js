...
function getDescriptionHtml(listing) {
  ...
  const paragraphs = listing.description
    ...
    : [
       `<p>${escapeHTML(listing.title)} listed by a Texas Tech student on TTU Marketplace.</p>`,
      `<p>Condition is ${escapeHTML(listing.condition)}. Available for pickup around ${escapeHTML(listing.location)}.</p>`
      ];

  const extras = listing.highlights
    ? `<ul>${listing.highlights.map((line) => `<li>${escapeHTML(line)}</li>`).join("")}</ul>`
    : `<ul>
...
if (!item) {
  ...
  document.querySelector("#sellerName").innerHTML = `${escapeHTML(seller.name)} <b>✓</b>`;
  ...
  thumbnailsContainer.innerHTML = images
    .map((image, index) => `
      <button class="thumbnail${index === 0 ? " active" : ""}" type="button" data-image="${escapeHTML(image.src)}" data-alt="${escapeHTML(image.alt)}">
