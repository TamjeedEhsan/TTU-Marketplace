...
function renderCart() {
  ...
  cartItems.innerHTML = cart
    .map((item) => `
      <article class="cart-item" data-id="${Number(item.id)}" data-price="${Number(item.price)}">
        <a class="product-image" href="product.html?id=${Number(item.id)}">
        ...
        <div class="product-info">
          ...
          <a href="product.html?id=${Number(item.id)}">
           <h2>${escapeHTML(item.title)}</h2>
          </a>
          <p>Seller: ${escapeHTML(item.seller)} <b>✓</b></p>
        ...
        <div class="item-price-column">
          ...
          <div class="quantity-control" aria-label="${escapeHTML(item.title)} quantity">
